...
/*
void save_to_Ablums_list(AlbumList_Struct& ALS) {
  // open the album folder                    (do this at intro() )
  // if not exist then create it "Album"      (do this at intro() )
  // Search for .alb file                     (do this at intro() )
  // Save into a Album_list.txt               (do this at intro() )
  // read Album_list.txt                      (do this at intro() )
  // put into array of strings ablum[100];

  // Albums_limit 100
  // Songs_per_Album_limit 100
  ...
  File AlbumFolder = SD_MMC.open("/Album");
  File Album = AlbumFolder.openNextFile();
  File save_to_list = SD_MMC.open("/Album/.Album_list.txt", FILE_WRITE);
  String extension = ".alb";
  uint8_t Album_count = 0;

  save_to_list.write(Album_count);  //set 1byte offset for later write

  while (Album) {
    const char* cAlb_name = Album.name();
    String name = cAlb_name;
    String checking = cAlb_name;

    if (checking.substring(checking.length() - 4, checking.length()) == extension) {
      ALS.Album_name[Album_count] = "/Album/" + name;  //save path to struct
      save_to_list.printf("/Album/%s\n", cAlb_name);   // save name to file
                                                       // Serial.println(ALS.Album_name[Album_count]);
                                                       // read_Ablums_listM(ALS.Album_name[Album_count]);

      //Serial.println(" ");
      Album_count++;
    }

    if (Album_count == Albums_limit) {
      break;
    }
    Album.close();
    Album = AlbumFolder.openNextFile();
  }

  if (Album_count == 0) {
    Serial.println("Found no Album file");
  }

  save_to_list.seek(0);
  save_to_list.write(Album_count);  // save amount of Albums

  AlbumFolder.close();
  Album.close();
  save_to_list.close();
}
uint8_t read_Ablum_Song_list(Album_Struct& AS, String Album_name) {
...
void read_Ablums_listM(String Album_name) {
  File Songs_in_Album = SD_MMC.open(Album_name, FILE_READ);
  String song_name;
  uint8_t count_song = 0;
  while (Songs_in_Album) {
    song_name = Songs_in_Album.readStringUntil('\n');
    count_song++;
    if (song_name == "" || count_song == Songs_per_Album_limit) {
      break;
    }
    // Serial.println(song_name);
  }
  // should return a amount of songs
}
